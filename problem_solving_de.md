...
Wir erstellen den Namen der Ziel-ZIP-Datei mit dem Additions-Operator (`+`), der Zeichenketten verbindet ( _to concatenate_),  und eine neue Zeichenkette zurückgibt. Dann erstellen wir eine Zeichenkette `zip_command`, die den Befehl enthält, den wir ausführen werden. Sie können prüfen, ob dieser Befehl funktioniert, indem Sie ihn in der Shell (GNU/Linux-Terminal oder DOS-Eingabeaufforderung) ausführen.
